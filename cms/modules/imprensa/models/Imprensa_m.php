...
class Imprensa_m extends CI_Model {
    ...
    public function get_imprensa($params = array())
    {
        $options = array(
            'texto' => "",
            'data_de' => false,
        ...
        $imprensa = $this->db->get();

        if($params['count']){
            $toReturn = $imprensa->num_rows();
        }else if($params['id']){
            $toReturn = $imprensa->row();
        }else{
            $toReturn = $imprensa->result();
        }
    ...
    public function atualizar($data, $dataWhere) {
        ...
        $this->db->where('id', $dataWhere['id'])->update('imprensa', $data);
    ...
    {
        if ($this->db->delete('imprensa', array('id' => $id))){
            return true;
        }else{
            return false;
        }
