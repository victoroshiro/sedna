...
class Imprensas extends CI_Controller {
    ...
    public function index($offset = NULL)
    {
        if ($this->Usuarios_m->logado()) {
            ...
            $this->data["imprensas_lista"] = $this->Imprensas_m->get_imprensas(
                array(
                    'texto' => $texto, 
                    'data_de' => $data_de, 
                    'data_ate' => $data_ate
    ...
    public function salvar()
    {
        ...
        if($this->Imprensas_m->salvar($data)){
    ...
    public function editar($id = false)
    {
        ...
        $this->data['imprensa'] = $this->Imprensas_m->get_imprensas(array('id' => $id));
    ...
    public function atualizar()
    {
        ...
        if ($this->Imprensas_m->atualizar($data, $dataWhere)) {
            $this->session->set_flashdata('messages', 'Registro atualizado com sucesso.');
            redirect('imprensas/editar/' .$id, 'location');
        } else {
    ...
    public function excluir_selecionados()
    {
        ...
        $imprensas = explode(';', $ids);
        
        for ($i = 0; $i <= count($imprensas) - 1; $i++) {

            if (!$this->Imprensas_m->excluir($imprensas[$i])) {
                $ok = false;
                $erros[] = $imprensas[$i];
            }
        }
        ...
        if ($ok) {
            ...
            for ($i = 0; $i <= count($erros) - 2; $i++) {
                $imprensa = $this->Imprensas_m->get_imprensas(array('id' => $erros[$i]));
                if ($i < count($erros) - 2)
                    $msg .= $imprensa->titulo . ", ";
                else
                    $msg .= $imprensa->titulo . ".";
