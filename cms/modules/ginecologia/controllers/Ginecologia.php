...
class Ginecologia extends CI_Controller {
    ...
    public function index($offset = NULL)
    {
        if ($this->Usuarios_m->logado()) {
            ...
            $this->data["gineco_lista"] = $this->Ginecologia_m->get_ginecologia(
                array(
                    'texto' => $texto, 
                    'data_de' => $data_de, 
                    'data_ate' => $data_ate
    ...
    public function salvar()
    {
        ...
        if($this->Ginecologia_m->salvar($data)){
    ...
    public function editar($id = false)
    {
        ...
        $this->data['gineco'] = $this->Ginecologia_m->get_ginecologia(array('id' => $id));
    ...
    public function atualizar()
    {
        ...
        if ($this->Ginecologia_m->atualizar($data, $dataWhere)) {
            $this->session->set_flashdata('messages', 'Registro atualizado com sucesso.');
            redirect('ginecologia/editar/' .$id, 'location');
        } else {
    ...
    public function excluir_selecionados()
    {
        ...
        $ginecologia = explode(';', $ids);
        
        for ($i = 0; $i <= count($ginecologia) - 1; $i++) {

            if (!$this->Ginecologia_m->excluir($ginecologia[$i])) {
                $ok = false;
                $erros[] = $ginecologia[$i];
            }
        }
        ...
        if ($ok) {
            ...
            for ($i = 0; $i <= count($erros) - 2; $i++) {
                $gineco = $this->Ginecologia_m->get_ginecologia(array('id' => $erros[$i]));
                if ($i < count($erros) - 2)
                    $msg .= $gineco->titulo . ", ";
                else
                    $msg .= $gineco->titulo . ".";
    ...
    private function _upload_foto($params = array())
    {
        ...
        $dir = dirname(getcwd()).'/userfiles/ginecologia/';
