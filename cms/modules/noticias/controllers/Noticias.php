...
class Noticias extends CI_Controller {
    ...
    public function index($offset = NULL)
    {
        if ($this->Usuarios_m->logado()) {
            ...
            $this->data["noticias_lista"] = $this->Noticias_m->get_noticias(
                array(
                    'texto' => $texto, 
                    'data_de' => $data_de, 
                    'data_ate' => $data_ate
    ...
    public function salvar()
    {
        ...
        if($this->Noticias_m->salvar($data)){
    ...
    public function editar($id = false)
    {
        ...
        $this->data['noticia'] = $this->Noticias_m->get_noticias(array('id' => $id));
    ...
    public function atualizar()
    {
        ...
        if ($this->Noticias_m->atualizar($data, $dataWhere)) {
            $this->session->set_flashdata('messages', 'Registro atualizado com sucesso.');
            redirect('noticias/editar/' .$id, 'location');
        } else {
    ...
    public function excluir_selecionados()
    {
        ...
        for ($i = 0; $i <= count($noticias) - 1; $i++) {
            ...
            if (!$this->Noticias_m->excluir($noticias[$i])) {
                $ok = false;
                $erros[] = $noticias[$i];
            }
        }
        ...
        if ($ok) {
            ...
            for ($i = 0; $i <= count($erros) - 2; $i++) {
                $noticia = $this->Noticias_m->get_noticias(array('id' => $erros[$i]));
                if ($i < count($erros) - 2)
                    $msg .= $noticia->titulo . ", ";
                else
                    $msg .= $noticia->titulo . ".";
