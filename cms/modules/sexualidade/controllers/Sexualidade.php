...
class Sexualidade extends CI_Controller {
    ...
    public function index($offset = NULL)
    {
        if ($this->Usuarios_m->logado()) {
            ...
            $this->data["sexualidade_lista"] = $this->Sexualidade_m->get_sexualidades(
                array(
                    'texto' => $texto, 
                    'data_de' => $data_de, 
                    'data_ate' => $data_ate
    ...
    public function salvar()
    {
        ...
        if($this->Sexualidade_m->salvar($data)){
    ...
    public function editar($id = false)
    {
        ...
        $this->data['sexualidade'] = $this->Sexualidade_m->get_sexualidades(array('id' => $id));
    ...
    public function atualizar()
    {
        ...
        if ($this->Sexualidade_m->atualizar($data, $dataWhere)) {
            $this->session->set_flashdata('messages', 'Registro atualizado com sucesso.');
            redirect('sexualidade/editar/' .$id, 'location');
        } else {
    ...
    public function excluir_selecionados()
    {
        ...
        $sexualidades = explode(';', $ids);
        
        for ($i = 0; $i <= count($sexualidades) - 1; $i++) {

            if (!$this->Sexualidade_m->excluir($sexualidades[$i])) {
                $ok = false;
                $erros[] = $sexualidades[$i];
            }
        }
        ...
        if ($ok) {
            ...
            for ($i = 0; $i <= count($erros) - 2; $i++) {
                $sexualidade = $this->Sexualidade_m->get_sexualidades(array('id' => $erros[$i]));
                if ($i < count($erros) - 2)
                    $msg .= $sexualidade->titulo . ", ";
                else
                    $msg .= $sexualidade->titulo . ".";
            }
    ...
    private function _upload_foto($params = array())
    {
        ...
        $dir = dirname(getcwd()).'/userfiles/sexualidades/';
