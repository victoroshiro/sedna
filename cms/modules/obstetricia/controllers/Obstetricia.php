...
class Obstetricia extends CI_Controller {
    ...
    public function index($offset = NULL)
    {
        if ($this->Usuarios_m->logado()) {
            ...
            $this->data["obstetricia_lista"] = $this->Obstetricia_m->get_obstetricias(
                array(
                    'texto' => $texto, 
                    'data_de' => $data_de, 
                    'data_ate' => $data_ate
    ...
    public function salvar()
    {
        ...
        if($this->Obstetricia_m->salvar($data)){
    ...
    public function editar($id = false)
    {
        ...
        $this->data['obstetricia'] = $this->Obstetricia_m->get_obstetricias(array('id' => $id));
    ...
    public function atualizar()
    {
        ...
        if ($this->Obstetricia_m->atualizar($data, $dataWhere)) {
            $this->session->set_flashdata('messages', 'Registro atualizado com sucesso.');
            redirect('obstetricia/editar/' .$id, 'location');
        } else {
    ...
    public function excluir_selecionados()
    {
        ...
        $obstetricias = explode(';', $ids);
        
        for ($i = 0; $i <= count($obstetricias) - 1; $i++) {

            if (!$this->Obstetricia_m->excluir($obstetricias[$i])) {
                $ok = false;
                $erros[] = $obstetricias[$i];
            }
        }
        ...
        if ($ok) {
            ...
            for ($i = 0; $i <= count($erros) - 2; $i++) {
                $obstetricia = $this->Obstetricia_m->get_obstetricias(array('id' => $erros[$i]));
                if ($i < count($erros) - 2)
                    $msg .= $obstetricia->titulo . ", ";
                else
                    $msg .= $obstetricia->titulo . ".";
            }
    ...
    private function _upload_foto($params = array())
    {
        ...
        $dir = dirname(getcwd()).'/userfiles/obstetricias/';
