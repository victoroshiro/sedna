...
(function($) {
  ...
  $.fn.RSV.validate = function(event)
  {
    ...
    // Alan - if the user has defined a customBeforeValidation, execute it
    if (typeof options.customBeforeValidation == 'function')
    	options.customBeforeValidation();
      ...
      switch (requirement)
      {
        ...
        case "function":
          ...
          if (row.length == 2)
        	  ...
        	  newrow = row.slice(2, row.length).join("','");
        	  eval("var result = " + custom_function + "('" + newrow + "')");
          }
